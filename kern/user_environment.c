...
DECLARE_START_OF(tst_krealloc_shares_realloc);
...
struct UserProgramInfo userPrograms[] = {
		...
		{ "tkreallocshares", "Test shares array re-allocation if no shares are available", PTR_START_OF(tst_krealloc_shares_realloc)},
...
void * create_user_page_WS(unsigned int numOfElements)
{
	...
	//void*addr=(void *)kmalloc(numOfElements*PAGE_SIZE);
	void*addr=(void *)kmalloc(numOfElements*sizeof(struct WorkingSetElement));
	return addr;
...
struct Env* env_create(char* user_program_name, unsigned int page_WS_size)
{
	...
	e->page_WS_max_size = page_WS_size;
     /* HEEEEEEEEEEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE*/
