...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if(env_page_ws_get_size(curenv)<curenv->page_WS_max_size)
     ...
     else
     {
         ...
         struct Frame_Info*fi=NULL;
         uint32 *ptr=NULL;
         fi=get_frame_info(curenv->env_page_directory,(void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,&ptr);
         if(pt_get_page_permissions(curenv,curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address)&PERM_MODIFIED)// modified
         {
            pf_update_env_page(curenv,(void *)curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address,fi);
