...
int test_kmalloc() {
	...
	void* ptr_allocations[20] = { 0 };
	{
		...
		uint32 restOfKHeap = (KERNEL_HEAP_MAX - ACTUAL_START + 2 * PAGE_SIZE)
				- (2 * Mega + 2 * Mega + 4 * kilo + 4 * kilo + 8 * kilo
						+ 3 * Mega + 6 * Mega + 16 * kilo);
...
int test_kmalloc_nextfit() {
	...
	if ((pf_calculate_free_frames() - freeDiskFrames) != 0)
		panic(
				"Page file is changed while it's not expected to. (pages are wrongly allocated/de-allocated in PageFile)");
	if ((sys_calculate_free_frames() - freeFrames)
			!= ((10 * 2 * Mega) - PAGE_SIZE - KERNEL_SHARES_ARR_INIT_SIZE
					- KERNEL_SEMAPHORES_ARR_INIT_SIZE) / PAGE_SIZE)
...
int test_kmalloc_firstfit1() {
	...
	cprintf(
			"Congratulations!! test FIRST FIT allocation (1) completed successfully.\n");
...
//		if ((sys_calculate_free_frames() - freeFrames) != 514) panic("Wrong free: ");
	...
	//	b) Attempt to allocate large segment with no suitable fragment to fit on
	{
		...
		cprintf(
				"Congratulations!! test FIRST FIT allocation (2) completed successfully.\n");
	}
	return 1;
}
...
int test_kheap_phys_addr() {
	...
	//test kheap_physical_address after kmalloc only [40%]
	{
		...
		uint32 allPAs[(13 * Mega + 32 * kilo + KERNEL_SHARES_ARR_INIT_SIZE
				+ KERNEL_SEMAPHORES_ARR_INIT_SIZE) / PAGE_SIZE];
		i = 0;
	...
	//test kheap_physical_address on non-mapped area [20%]
	{
		...
		{
			uint32 *ptr_table;
			get_page_table(ptr_page_directory, (void*) (uint32) va2,
					&ptr_table);
			if (ptr_table == NULL) {
				panic(
						"one of the kernel tables is wrongly removed! Tables of Kernel Heap should not be removed");
...
int test_kheap_virt_addr() {
	...
	uint32 allPAs[(13 * Mega + 32 * kilo + KERNEL_SHARES_ARR_INIT_SIZE
			+ KERNEL_SEMAPHORES_ARR_INIT_SIZE) / PAGE_SIZE];
	int numOfFrames = (13 * Mega + 32 * kilo + KERNEL_SHARES_ARR_INIT_SIZE
			+ KERNEL_SEMAPHORES_ARR_INIT_SIZE) / PAGE_SIZE;
	...
	//test kheap_virtual_address after kmalloc and kfree [40%]
	{
		...
		uint32 startIndex = (KERNEL_SHARES_ARR_INIT_SIZE
				+ KERNEL_SEMAPHORES_ARR_INIT_SIZE) / PAGE_SIZE;
		...
		for (i = startIndex + 4 * Mega / PAGE_SIZE;
				i < startIndex + (7 * Mega + 16 * kilo) / PAGE_SIZE; ++i) {
			uint32 retrievedVA = kheap_virtual_address(allPAs[i]);
		...
		//frames of 6 MB
		for (i = startIndex + (7 * Mega + 16 * kilo) / PAGE_SIZE;
				i < startIndex + (13 * Mega + 16 * kilo) / PAGE_SIZE; ++i) {
			uint32 retrievedVA = kheap_virtual_address(allPAs[i]);
			if (retrievedVA != 0) {
				panic("Wrong kheap_virtual_address");
			}
		...
		for (i = startIndex + (13 * Mega + 16 * kilo) / PAGE_SIZE;
				i < startIndex + (13 * Mega + 32 * kilo) / PAGE_SIZE; ++i) {
			uint32 retrievedVA = kheap_virtual_address(allPAs[i]);
			if (retrievedVA != KERNEL_HEAP_START + i * PAGE_SIZE) {
				panic("Wrong kheap_virtual_address");
			}
