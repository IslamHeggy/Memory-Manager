...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	int ret2=get_share_object_ID(ownerID,shareName);
	if(ret2>=0)
		return E_SHARED_MEM_EXISTS ;
    struct Share *Newobj;
	int ret=allocate_share_object(&Newobj);
	if(ret==E_NO_SHARE)
		return ret ;
	struct Frame_Info *ptr_frame_info ;
	size=ROUNDUP(size,PAGE_SIZE);

	uint32 tmp_va=(uint32)virtual_address;
	for(int i =0; i<(size/PAGE_SIZE); i++)
	{
	allocate_frame(&ptr_frame_info);
	map_frame(curenv->env_page_directory,ptr_frame_info,(void*)tmp_va,PERM_WRITEABLE|PERM_USER);
	add_frame_to_storage(Newobj->framesStorage,ptr_frame_info,i);
	tmp_va+=PAGE_SIZE;
	}
	strcpy(Newobj->name, shareName);
	Newobj->ownerID=ownerID;
	Newobj->size=size;
	Newobj->isWritable=isWritable;
	Newobj->references=1;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	if(Shared_ID==E_SHARED_MEM_NOT_EXISTS)
		return Shared_ID;
	uint32 tmp_va=(uint32)virtual_address;
	for(int i=0;i<(shares[Shared_ID].size/PAGE_SIZE);i++)
	{
ptr_frame_info= get_frame_from_storage(shares[Shared_ID].framesStorage,i);
if(shares[Shared_ID].isWritable)
map_frame(myenv->env_page_directory,ptr_frame_info,(void *)tmp_va,PERM_WRITEABLE|PERM_USER);
else
map_frame(myenv->env_page_directory,ptr_frame_info,(void *)tmp_va,PERM_USER);
tmp_va+=PAGE_SIZE;
	}
shares[Shared_ID].references++;
return Shared_ID;
}
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	...
	//change this "return" according to your answer
		...
		for(int i =0 ; i<MAX_SHARES ; i++)
		{
			...
			uint32 *ptr=NULL;
			struct Frame_Info * ptr_frame_info1=get_frame_info(curenv->env_page_directory,(void *)temp,&ptr); ;
	...
	if(shared_Index==-1)
	{
		...
		//	6) Flush the cache "tlbflush()"
		temp = (uint32 )startVA;
		for(int i=0;i<shares[shared_Index].size/PAGE_SIZE;i++)
		{
			unmap_frame(myenv->env_page_directory,(void *)temp);
			temp+=PAGE_SIZE;
		}
		 temp=(uint32)startVA;
		       for(int i=0;i<(shares[shared_Index].size/PAGE_SIZE);i++)
		       {
		           uint8 found=1;
		        uint32*ptr=NULL;
		        get_page_table(myenv->env_page_directory,(void*)temp,&ptr);
		            if(ptr!=NULL)
		            {
		           ...
		           {
		        	   kfree((void *)ptr);
		        	   myenv->env_page_directory[PDX(temp)]=0;
		           }
		           }
		         temp+=PAGE_SIZE;
		       }
