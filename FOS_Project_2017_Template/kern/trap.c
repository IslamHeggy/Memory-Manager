...
void fault_handler(struct Trapframe *tf)
{
	...
	if ( (curenv->env_page_directory[PDX(fault_va)] & PERM_PRESENT) != PERM_PRESENT)
	{
		...
		//		cprintf("[%s] user TABLE f ault va %08x\n", curenv->prog_name, fault_va);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	//cprintf("virtual add in page fault=%x\n",fault_va);
	...
	if(env_page_ws_get_size(curenv)<curenv->page_WS_max_size)
     {
		// fifo 2 uncomment it when test fifo2
		 /*uint32 minn=0xfffffff0;
		    for(int j=0;j<curenv->page_WS_max_size;j++)
		       {
		      	 if(curenv->ptr_pageWorkingSet[j].time_stamp<minn)
		       	 {
	        		 minn=curenv->ptr_pageWorkingSet[j].time_stamp;
	        	     curenv->page_last_WS_index=j;
		         }
		       }
		  */
    	 ...
    	 if(fi==NULL)
    	 {
    		 panic("No Free farmes is exist!!");
    	 }
    	 map_frame(curenv->env_page_directory,fi,(void *)fault_va,PERM_USER |PERM_WRITEABLE);
    	 if(pf_read_env_page(curenv,(void *)fault_va)!=0)
    	 {
           ...
           {
        	   panic("invalid access in page file \n!!!..");
           }
    	 }
     ...
     else
     {
         struct Frame_Info*fi=NULL;
         uint32 *ptr=NULL;
         //TODO: [PROJECT 2017 - BONUS8] fifo2 [Kernel Side]
         // fifo 2 uncomment it when test fifo2
         uint32 victim_va,minn=0xfffffff0;
        /* for(int j=0;j<curenv->page_WS_max_size;j++)
         {
        	 if(curenv->ptr_pageWorkingSet[j].time_stamp<minn)
        	 {
        		 minn=curenv->ptr_pageWorkingSet[j].time_stamp;
        		 victim_va=curenv->ptr_pageWorkingSet[j].virtual_address;
        		 curenv->page_last_WS_index=j;
        	 }
         }*/
         victim_va=curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;
         fi=get_frame_info(curenv->env_page_directory,(void *)victim_va,&ptr);
         if(pt_get_page_permissions(curenv,victim_va)&PERM_MODIFIED)// modified
         {
