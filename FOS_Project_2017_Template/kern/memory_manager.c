...
struct Block_Info blocks_info[Max_number_of_blocks];
struct Linked_List_block Block_info_free_list;     // Free list of physical frames_info
struct Linked_List_block Block_info_taken_list;    // taken list of physical frames_info
struct Linked_List_block free_blocks;
...
void initialize_paging()
{
	...
	//kheap arrays
	LIST_INIT(&Block_info_free_list);
	LIST_INIT(&Block_info_taken_list);
	LIST_INIT(&free_blocks);
	for(int i=0;i<Max_number_of_blocks;i++)
		LIST_INSERT_HEAD(&free_blocks,&blocks_info[i]);
	struct Block_Info *ptr;
	k_allocate_block(&ptr);
	ptr->start_va=KERNEL_HEAP_START+0x4000;
	ptr->size=KERNEL_HEAP_MAX-(KERNEL_HEAP_START+0x4000);
	LIST_INSERT_HEAD(&Block_info_free_list,ptr);
	int i;
...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
    ...
    uint32*add=kmalloc(PAGE_SIZE);
    if(add==NULL)
    {
    	panic("No enough memory");
    }
    ptr_page_directory[PDX(virtual_address)]=
    		CONSTRUCT_ENTRY((uint32)(kheap_physical_address((unsigned int )add)),PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
...
//2. Free ONLY pages that are resident in the working set from the memory
    for(int j=0;j<e->page_WS_max_size;j++)
     {
  if(env_page_ws_get_virtual_address(e,j)>=virtual_address && env_page_ws_get_virtual_address(e,j)<virtual_address+size)
      {
        unmap_frame(e->env_page_directory,(void *)env_page_ws_get_virtual_address(e,j));
        env_page_ws_clear_entry(e,j);
        if(j<e->page_last_WS_index)
        {
  ...
  //3. Removes ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)
    tmp=virtual_address;
       for(int i=0;i<(size/PAGE_SIZE);i++)
       {
           uint8 found=1;
        uint32*ptr=NULL;
        get_page_table(e->env_page_directory,(void*)tmp,&ptr);
            if(ptr!=NULL)
            {
           ...
           {
        	   kfree((void *)ptr);
        	   e->env_page_directory[PDX(tmp)]=0;
           }
           }
         tmp+=PAGE_SIZE;
       }
...
void moveMem(struct Env* e, uint32 src_virtual_address, uint32 dst_virtual_address, uint32 size)
{
	...
	// or main memory
	uint32 tmp_src=(uint32)src_virtual_address;
	uint32 tmp_dst=(uint32)dst_virtual_address;
	for(int i=0;i<(size/PAGE_SIZE);i++)
	 {
	   pf_add_empty_env_page(e,tmp_dst,0);
	   pf_remove_env_page(e,tmp_src);
	   tmp_src+=PAGE_SIZE;
	   tmp_dst+=PAGE_SIZE;
	 }
	for(int j=0;j<e->page_WS_max_size;j++)
	     {
		uint32 tmp_va=env_page_ws_get_virtual_address(e,j);
	  if(tmp_va>=src_virtual_address && tmp_va<src_virtual_address+size)
	      {
	        unmap_frame(e->env_page_directory,(void *)tmp_va);
	       e->ptr_pageWorkingSet[j].virtual_address=dst_virtual_address+(tmp_va-src_virtual_address);
	      }
	     }
	       for(int i=0;i<(size/PAGE_SIZE);i++)
	       {
	           uint8 found=1;
	        uint32*ptr1=NULL;
	        get_page_table(e->env_page_directory,(void*)src_virtual_address,&ptr1);
	        uint32*ptr2=NULL;
	        get_page_table(e->env_page_directory,(void*)dst_virtual_address,&ptr2);
	            if(ptr1!=NULL)
	            {
	            for(int k=0;k<1024;k++)
	           {
	           ...
	           {
	        	   kfree((void *)ptr1);
	        	   e->env_page_directory[PDX(src_virtual_address)]=0;
	           }
	           }
...
void k_allocate_block(struct Block_Info** curr)
{
 *curr=LIST_FIRST(&free_blocks);
 if(*curr==NULL)
