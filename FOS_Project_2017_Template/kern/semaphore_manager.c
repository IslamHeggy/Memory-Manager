...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	for (int i = 0 ; i < MAX_SEMAPHORES; ++i)
	{
			if (semaphores[i].empty != 1 && semaphores[i].ownerID == ownerEnvID && strcmp(semaphores[i].name,semaphoreName) == 0)
			{
				semaphores[i].value--;
				int32 val = semaphores[i].value;
				if (val < 0) {
					sched_remove_ready(myenv);
					enqueue(&semaphores[i].env_queue, myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	for (int i = 0 ; i < MAX_SEMAPHORES; ++i) {
			...
			{
				struct Env* newenv;
				newenv=dequeue(&semaphores[i].env_queue);
